        ...
        implements PlatformView
{
    ...
    public static MaxAdView getInstance(final String placement)
    {
        AppLovinMAXAdViewWidget widget = preloadedWidgetInstances.get( placement );
        if ( widget == null ) widget = widgetInstances.get( placement );
    ...
    {
        AppLovinMAXAdViewWidget preloadedWidget = preloadedWidgetInstances.get( placement );
        if ( preloadedWidget != null )
        {
            Log.e( AppLovinMAX.TAG, "Cannot preload more than once for a single Ad Placement.", null );
            return;
        }

        preloadedWidget = new AppLovinMAXAdViewWidget( adUnitId, adFormat, true, sdk, context );
        preloadedWidgetInstances.put( placement, preloadedWidget );
    ...
    public static void destroyWidgetAdView(final String placement, final Result result)
    {
        AppLovinMAXAdViewWidget preloadedWidget = preloadedWidgetInstances.get( placement );
        if ( preloadedWidget == null )
        {
            Log.e( AppLovinMAX.TAG, "No widget found to destroy", null );
            return;
        }

        if ( preloadedWidget.hasContainerView() )
        {
            Log.e( AppLovinMAX.TAG, "Cannot destroy - currently in use", null );
            return;
        }

        preloadedWidgetInstances.remove( placement );
                             ...
                             final Context context)
    {
        ...
        widget = preloadedWidgetInstances.get( placement );
        if ( widget != null )
        {
        ...
        widgetInstances.put( placement, widget );
    ...
    public void dispose()
    {
        if ( widget != null )
        {
            ...
            AppLovinMAXAdViewWidget preloadedWidget = preloadedWidgetInstances.get( widget.getAdView().getPlacement() );

            if ( widget != preloadedWidget )
            {
                widgetInstances.remove( widget.getAdView().getPlacement() );
                widget.destroy();
            }
        }
